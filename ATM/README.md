...
The user enters their card number and PIN, which are verified against the database. If the credentials are valid, the user is redirected to the ATM menu. If invalid details are entered, an error message is shown.



The CLEAR button allows users to reset the entered credentials.

New users can register by clicking the “NEW USER? REGISTER HERE” button, which redirects them to the signup process.
